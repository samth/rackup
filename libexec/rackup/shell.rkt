...
(define (zsh-completion-script)
  (string-append
   ...
   "      ;;\n"
   "    install)\n"
   "      _arguments '*:option:(stable pre-release snapshot snapshot:utah snapshot:northwestern --variant --distribution --snapshot-site --arch --set-default --force --no-cache --quiet --verbose)'\n"
   "      ;;\n"
   "    link)\n"
