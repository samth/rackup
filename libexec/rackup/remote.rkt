...
(define (status-code headers)
  (define h (if (bytes? headers) (bytes->string/utf-8 headers) headers))
...
(define (http-open url-str)
  ...
  (unless (equal? code 200)
    (close-input-port in)
    (rackup-error "HTTP request failed (~a): ~a" code (if (url? url-str) (url->string url-str) url-str)))
  in)
...
(define (fetch-snapshot-stamp site)
  (http-get-string (format "~a/snapshots/current/stamp.txt"
                           (hash-ref snapshot-sites site))))

(define installer-rx
  #px"^(racket(?:-minimal)?)-([^-]+)-(.+?)(?:-(bc|cs))?\\.([A-Za-z0-9]+)$")
...
(define (parse-installer-filename s)
  (match (regexp-match installer-rx s)
    [(list _ prefix version-token platform-token variant-s ext)
     ...
     (define platform-parts (if (>= (length parts) 2) (cdr parts) null))
     (define platform (if (pair? platform-parts)
                          (string-join platform-parts "-")
                          ""))
     (define platform-family (if (pair? platform-parts) (car platform-parts) platform))
     (hash 'filename s
...
(define (table-filenames table)
  (cond
    [(hash? table)
     (for/list ([v (in-hash-values table)] #:when (string? v))
       v)]
    [(list? table)
     (filter string? table)]
    [else
...
(define (select-installer-filename table
    ...
    (or (equal? token wanted)
        (and allow-version-prefix?
             (regexp-match? (pregexp (string-append "^"
                            ...
                            token))))
  (define parsed
    (filter values (map parse-installer-filename (table-filenames table))))
  (define (normal-platform-installer? p)
    ...
    (cond
      ...
      [else
       (equal? (hash-ref p 'platform-family (hash-ref p 'platform "")) platform)]))
  (define matches
    (for/list ([p parsed]
  ...
  (cond
    ...
    [else
     (rackup-error "no installer found in table for version-token=~a variant=~a distro=~a arch=~a platform=~a ext=~a"
                   version-token variant distribution arch platform ext)]))
...
(define (normalize-site-option spec-site cli-site)
  ...
  (define (valid-site? s) (member s '(auto utah northwestern)))
...
(define (try-resolve-snapshot-site sites
                                   #:distribution distribution
                                   #:arch arch
                                   #:variant variant)
  (define results
  ...
  (car (sort ok-results
             (lambda (a b)
               (string>? (hash-ref a 'stamp "") (hash-ref b 'stamp ""))))))
...
(define (resolve-install-request spec
  ...
  (define spec* (if (string? spec) (parse-install-spec spec) spec))
  ...
  (match kind
    ...
    ['pre-release
     ...
     (hash 'kind 'pre-release
           ...
           'snapshot-site #f
           'snapshot-stamp #f
           'installers-base pre-release-installers-base
           'installer-filename filename
           'installer-url (string-append pre-release-installers-base filename))]
    ['snapshot
     ...
     (define variant (if variant-override (parse-variant variant-override) 'cs))
     (define picked
