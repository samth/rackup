...
(define (parse-download-page-checksums html-str)
  ;; Associate each installer with the checksum in its *own* row: the
  ;; first <span class="checksum">HEX</span> that appears after that
  ;; installer's <a ... href="installers/FILENAME"> and before the next
  ;; installer href.  Installers with no checksum before the next href
  ;; get no entry.
  ;;
  ;; Do NOT zip a flat list of filenames against a flat list of checksums
  ;; by position: the Racket *snapshot* pages list many installers but
  ;; only some of them carry a checksum span (e.g. 134 installers, 52
  ;; checksums), so a positional zip silently pairs installers with the
  ;; wrong file's checksum.  Release pages happen to be 1:1, which hid the
  ;; bug there.
  (define href-rx #px"href=\"installers/([^\"]+)\"")
  (define sum-rx #px"class=\"checksum\"[^>]*>([0-9a-fA-F]{40,64})<")
  (define filenames (regexp-match* href-rx html-str #:match-select cadr))
  (define href-spans (list->vector (regexp-match-positions* href-rx html-str)))
  (define n (vector-length href-spans))
  (define html-len (string-length html-str))
  (for/fold ([acc (hash)])
            ([filename (in-list filenames)]
             [i (in-naturals)])
    ;; Search only between this href and the next one, so a checksum can
    ;; only be claimed by the installer whose row it lives in.
    (define region-start (cdr (vector-ref href-spans i)))
    (define region-end
      (if (< (add1 i) n) (car (vector-ref href-spans (add1 i))) html-len))
    (cond
      [(regexp-match sum-rx html-str region-start region-end)
       => (lambda (m)
            (define hex (string-downcase (cadr m)))
            (hash-set acc filename
