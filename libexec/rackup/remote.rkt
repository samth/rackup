...
(define (parse-download-page-checksums html-str)
  ;; Extract two parallel lists from the download page HTML:
  ;; 1. Installer filenames from <a class="installer..." href="installers/FILENAME">
  ;; 2. Checksum hex values from <span class="checksum">HEX</span>
  ;; These appear in the same order on the page (generated by distro-build),
  ;; so we zip them. No HTML parser needed — just two targeted extractions.
  (define installer-filenames
    (for/list ([m (regexp-match* #px"href=\"installers/([^\"]+)\""
                                 html-str #:match-select cadr)])
      m))
  (define checksum-values
    (for/list ([m (regexp-match* #px"class=\"checksum\"[^>]*>([0-9a-fA-F]{40,64})<"
                                 html-str #:match-select cadr)])
      (string-downcase m)))
